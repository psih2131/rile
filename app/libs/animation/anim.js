...
Skroll = function (opt) {
    ...
    this.animations = {
        ...
        rotate90: {
            start: function (el) {
                el.style["transform"] = "translate(0%,0) rotate(-90deg)";
                el.style["opacity"] = 1;
            },
            end: function (el) {
        ...
        fadeInUp_And_AnimNum: {
            start: function (el) {
                ...
                let spanElement = el.querySelector('.anim-number')
                increaseText(spanElement);
            }
        },
...
function increaseText(element) {
    if (element) {
        var targetNumber = parseInt(element.dataset.number);
        var currentNumber = parseInt(element.textContent);
        var increment = Math.ceil((targetNumber - currentNumber) / 20); // Размер инкремента для плавного увеличения текста
        ...
        }, 50); // Интервал времени между каждым обновлением текста (в миллисекундах)
    }
